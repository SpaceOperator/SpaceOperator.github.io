...
$.extend($.ui.rlightbox, {
	global: {
						...
						function( index, regExp ) {
							...
							if ( _res !== null ) {
								...
								if ( content.type === "image" || content.type === "flash" || thisElement.options.overwriteTitle ) {
									...
									_result.subtitle = $anchor.attr( "subtitle" );
									_result.url = _url;
								}
			...
			.success(
				function( data ) {
					...
					if ( _options.overwriteTitle === false ) {
						...
						_currentElement.subtitle = data.subtitle;
					}
				...
				function( json ) {
					...
					$content
						...
						if ( _options.overwriteTitle === false ) {
							...
							_currentElement.subtitle = json.data.subtitle;
						}
		...
		setReferences: function() {
			...
			$lb.headerSubWrapper = $lb.header.find( "#ui-lightbox-subtitle-wrapper" );			
			...
			$lb.subtitle = $lb.root.find( "#ui-lightbox-subtitle" );
		...
		updateSubTitle: function() {
			...
			// set new label for the subtitle and trim it if it is too long - no scrolling at the moment
			// 20px is a safety distance between text and the close button
			if ( _currentElement.subtitle !== "" ) {
				$lb.subtitle
					.empty()
					.append( _currentElement.subtitle );
			} else {

				// keep the line height – prevent counter from popping up in the subtitle line
				$lb.subtitle.append( "&nbsp;" );
			}
		},	 
		...
		updateSubTitleWidth: function() {
			var $lb = this.$lightbox;

			// 12px – 2 × border + 2 × padding
			$lb.headerSubWrapper.width( $lb.content.width() - 12*4 );
		},
			...
			htmlLightbox: "" +
				"<div id='ui-lightbox' class='ui-widget ui-widget-content ui-corner-all' style='display: none'>" +
					...
					"<div id='ui-lightbox-bottombar' class='ui-widget-header ui-corner-all' style='display: none'>" +
						...
						"</p>" +
						"<p id='ui-lightbox-subtitle-wrapper'>" +
							"<span id='ui-lightbox-subtitle'></span>" +
						"</p>" +
